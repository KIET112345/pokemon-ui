...
export class PokemonEffects {
  constructor(private actions$: Actions, private api: PokemonService, private store: Store<AppState>, private router: Router) {}

  initFromUrl$ = createEffect(() => this.actions$.pipe(
  ...
  load$ = createEffect(() => this.actions$.pipe(
    ...
    switchMap(([_, s]) => this.api.list({
      page: s.page, limit: s.pageSize, name: s.name, type: s.type,
      legendary: s.legendary, speedMin: s.speedMin, speedMax: s.speedMax
    }).pipe(
  ...
  syncUrl$ = createEffect(() => this.actions$.pipe(
    ...
    tap(([_, s]) => {
      ...
      const url = this.router.createUrlTree(['/pokemon'], { queryParams }).toString();
  ...
  importCsv$ = createEffect(() => this.actions$.pipe(
    ofType(P.importCsv),
    switchMap(({ file }) => this.api.importCsv(file).pipe(
